...
const DraggableDancer = ({ dancer, isAssigned, conflicts, onConflictClick }) => {
...
const DroppableScheduleSlot = ({
  ...
  const assignedDancers = schedule.assignedDancers
    .map((id) => dancers.find((d) => String(d.id) === String(id)))
    .filter(Boolean)
...
const DragDropScheduleManager = ({
  ...
  // Loading state moved to render loop to obey Rules of Hooks
  ...
  const assignDancerToSchedule = useCallback(
    (scheduleId, dancerId) => {
  ...
  const removeDancerFromSchedule = useCallback(
    (scheduleId, dancerId) => {
      setLocalSchedules((prevSchedules) => {
        let updatedSchedule = null
        const newSchedules = prevSchedules.map((schedule) => {
          if (schedule.id === scheduleId) {
            updatedSchedule = {
              ...schedule,
              assignedDancers: schedule.assignedDancers.filter(
                (id) => String(id) !== String(dancerId)
              ),
            }
            return updatedSchedule
          }
  ...
  // Update conflicts whenever schedules change - useMemo to avoid cascading renders
  const localConflicts = React.useMemo(() => {
    return detectConflicts(localSchedules)
  ...
  return (
    <div className="drag-drop-schedule-manager bg-background rounded-lg shadow-sm p-6 mb-6 border border-gray-200">
      ...
      <div className="schedule-container grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="conflicts-panel col-span-1 lg:col-span-3 bg-red-50 p-4 rounded-lg border border-red-100 mt-6">
          <h4 className="font-bold text-red-800 mb-3 flex items-center gap-2">
            ⚠️ Scheduling Conflicts ({localConflicts.length})
          </h4>
      ...
      <div className="schedule-summary mt-6 pt-4 border-t border-gray-200 flex gap-6 text-sm text-gray-600">
        ...
        <div>
          Total Events: <span className="font-bold">{localSchedules.length}</span>
        </div>
        <div>
          Total Dancers: <span className="font-bold">{dancers.length}</span>
        </div>
        <div>
          Assigned Dancers:{' '}
          <span className="font-bold">
            {localSchedules.reduce((sum, schedule) => sum + schedule.assignedDancers.length, 0)}
          </span>
        </div>
      </div>
    </div>
